...
import MapView, { PROVIDER_GOOGLE, Marker, Callout } from 'react-native-maps'; // Removed Circle
...
const AnimatedCircle = ({ radius, mapHeight, delta }: { radius: number, mapHeight: number, delta: Animated.SharedValue<number> }) => {
    const animatedStyle = useAnimatedStyle(() => {
        // Diameter (Pixels) = (RadiusKm * 2 * ScreenHeightPixels) / (LatitudeDeltaDegrees * 111)
        // 111 is approx km per degree.
        // Clamp delta to avoid division by zero
        const safeDelta = Math.max(delta.value, 0.0001);
        const diameter = (radius * 2 * mapHeight) / (safeDelta * 111);

        return {
            width: diameter,
            height: diameter,
            borderRadius: diameter / 2,
            transform: [{ scale: 1 }] // Can add entrance animation if wanted
        };
    });
                ...
                <View style={{ flex: 1 }}>
                    <ClusteredMapView
                        ...
                        onRegionChange={(region) => {
                            // Live update for Circle Size
                            currentDelta.value = region.latitudeDelta;
                        }}
                    ...
                    >
                        ...
                        {/* RADIUS CIRCLE (Reanimated Overlay) is now outside the map */}
                    </ClusteredMapView>

                    {/* FIXED CENTER OVERLAY (Circle + Crosshair) */}
                    {showFilterSheet && (
                        <View
                            pointerEvents="none"
                            style={[
                        ...
                        >
                            ...
                            {/* Center Crosshair */}
                            <View style={{ position: 'absolute', paddingBottom: SHEET_HEIGHT }}>
                ...
                <View className="absolute bottom-0 left-0 right-0 bg-white rounded-t-3xl shadow-2xl p-6 pb-10 z-30 animate-in slide-in-from-bottom-10">
                    ...
                    <View className="mb-8">
                        ...
                        <Slider
                            ...
                            onValueChange={(val) => {
                                setFilterRadius(val);
                                // Zoom map to fit radius in the fixed circle
                                // Circle size is now 0.7 * width
                                // We want 'val' km to roughly fill 0.35 * width (radius visual)
                                // Heuristic tuning:
                                const newDelta = val * 0.06; // Increased delta slightly to zoom out more (fit inside smaller circle)
                                mapRef.current?.animateToRegion({
                                    latitude: filterLocation?.latitude || 46.2044,
                                    longitude: filterLocation?.longitude || 6.1432,
