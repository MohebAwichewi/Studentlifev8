...
export default function UniversitiesPage() {
  ...
  // --- 2. GOOGLE MAPS SETUP (WEB COMPONENT) ---
  useEffect(() => {
    if (searchInputRef.current) {
      const el = searchInputRef.current as any;
      const onPlaceSelect = (event: any) => {
        const place = event.detail.getPlace();

        // Safe extraction for Web Component Place result
        // Similar logic: check location or geometry
        let lat = '';
        let lng = '';
          ...
          <div className="bg-white p-8 rounded-[2rem] w-full max-w-md shadow-2xl relative">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                {/* @ts-ignore */}
                <gmp-place-autocomplete ref={searchInputRef} types={['university', 'school']} placeholder="Type name...">
                  <input type="text" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl font-medium focus:ring-2 focus:ring-slate-900 outline-none" />
