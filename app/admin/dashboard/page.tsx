                  ...
                  {data.map((item: any, i: number) => (
                    <tr key={item.id || i} className="group hover:bg-slate-50/50 transition-colors">
                      <td className="px-8 py-5 font-bold text-slate-900">
                        {item.businessName || item.fullName || item.title || (item.business ? item.business.businessName : "System")}
                      </td>
                      ...
                      <td className="px-8 py-5 text-sm">
                        <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                           (item.status === 'ACTIVE' || item.status === 'APPROVED' || item.status === 'SENT' || item.isVerified) ? 'bg-emerald-100 text-emerald-600' : 
