...
export default function AdminDashboard() {
  ...
  const fetchOverviewData = async () => {
    try {
      // ✅ UPDATED PATH: /api/auth/admin/stats
      ...
      if (data.stats) {
        setStats({
            ...
            activeStudents: data.stats.totalStudents,
            pendingRequests: 0 
        })
  ...
  const handleStatusUpdate = async (id: number, newStatus: string) => {
    ...
    const actionMap: any = { 'APPROVED': 'APPROVE', 'REJECTED': 'BAN' }
    
    // ✅ UPDATED PATH
    await fetch('/api/auth/admin/action', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id, action: actionMap[newStatus] || 'BAN' })
    })
    fetchOverviewData()
  }
        ...
        {activeTab === 'overview' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <section className="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden mb-10">
              <div className="p-8 border-b border-slate-100"><h3 className="text-lg font-bold text-slate-900">Partner Management</h3></div>
              {loading ? <div className="p-8 text-center">Loading...</div> : applications.length === 0 ? <div className="p-12 text-center">No partners.</div> : (
                <table className="w-full text-left">
                  <thead className="bg-slate-50/50 text-xs font-bold text-slate-400 uppercase tracking-wider"><tr><th className="px-8 py-4">Business</th><th className="px-8 py-4">Plan</th><th className="px-8 py-4">Status</th><th className="px-8 py-4 text-right">Actions</th></tr></thead>
                  <tbody className="divide-y divide-slate-50">
                    {applications.map((app) => (
                      <tr key={app.id} className="group hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-8 py-5"><span className={`text-[10px] font-bold px-2 py-0.5 rounded ${app.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>{app.status}</span></td>
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
             ...
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
                   ...
                   <div className="overflow-x-auto">
                      <table className="w-full text-left">
                         <thead className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-slate-50">
                            <tr><th className="pb-3">Name</th><th className="pb-3">University</th><th className="pb-3 text-right">Status</th></tr>
                         </thead>
                         <tbody className="divide-y divide-slate-50">
                            {userAnalytics.recent.map((stu: any) => (
                               <tr key={stu.id}>
