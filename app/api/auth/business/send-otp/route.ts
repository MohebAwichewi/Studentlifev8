...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { data, error } = await resend.emails.send({
      ...
      text: `Your WIN verification code is: ${otp}. Please enter this code to verify your account. If you did not request this, please ignore this email.`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 10px;">
          ...
          <p style="color: #888; font-size: 12px; text-align: center;">Sent securely from WIN System.</p>
        </div>
      `
    });
