    ...
    return (
        <div className="min-h-screen bg-white flex items-center justify-center p-4 font-sans text-slate-900 selection:bg-black selection:text-white">
            <div className="w-full max-w-md text-center relative">
                ...
                <Link href="/user/login" className="absolute top-0 left-0 text-slate-400 hover:text-black transition p-2 hover:bg-slate-50 rounded-full">
                ...
                {status === 'SUCCESS' ? (
                    <div className="bg-green-50 text-green-800 p-8 rounded-2xl animate-in zoom-in border border-green-100">
                        ...
                        <Link href="/user/login" className="block mt-6 text-sm font-bold underline hover:text-green-950 transition">Back to Login</Link>
                    </div>
                ) : (
                    <form onSubmit={handleSubmit} className="space-y-6 text-left">
                        <div>
                            <label className="block text-xs font-bold text-slate-900 uppercase tracking-wider mb-2 ml-1">User Email</label>
                            <input
                                type="email"
                                required
