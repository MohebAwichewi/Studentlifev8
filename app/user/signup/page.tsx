        ...
        <div className="relative z-10">
          ...
          <div className="text-center mb-8">
            <Link href="/" className="inline-flex items-center gap-1 mb-4 group hover:opacity-80 transition">
              <span className="text-3xl font-black tracking-tighter text-slate-900">WIN</span>
            </Link>

            {step === 'form' ? (
              <>
                <h2 className="text-2xl font-black text-slate-900 mb-1">Join the Network</h2>
                <p className="text-slate-500 text-sm font-medium">Create your verified ID.</p>
              </>
            ) : (
              <>
          ...
          {step === 'form' && (
            <form onSubmit={handleGetMagicLink} className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div>
                ...
                <input required type="email" placeholder="name@example.com" className="w-full bg-[#F8F9FC] border border-slate-200 rounded-xl px-4 py-3.5 font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#FF3B30] transition" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} />
              </div>
              <div>
          ...
          {step === 'form' && (
            <div className="text-center mt-6 pt-6 border-t border-slate-50">
              <p className="text-sm text-slate-400 font-medium">Already verified? <Link href="/user/login" className="text-[#5856D6] font-bold hover:underline">Log in</Link></p>
            </div>
          )}
