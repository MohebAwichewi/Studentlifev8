...
export default function BusinessProfile() {
    ...
    // Prepare pins for the map
    let mapPins = business.locations.map((loc) => ({
        id: loc.id,
        lat: loc.lat,
        lng: loc.lng,
    ...
    // FALLBACK: If no locations but business has coordinates (legacy support)
    // We check for type because 0 is falsy, but valid coordinate (though unlikely exactly 0)
    if (mapPins.length === 0 && (business.latitude !== undefined && business.latitude !== 0)) {
        mapPins = [{
            id: 99999, // Dummy ID
    ...
    return (
        <div className="min-h-screen bg-white font-sans pb-32">
            ...
            <div className="max-w-5xl mx-auto px-6 -mt-16 relative z-10">
                <div className="bg-white rounded-3xl p-8 shadow-xl border border-slate-100 flex flex-col md:flex-row items-start md:items-end gap-6">
                    ...
                    <div className="flex-1">
                        ...
                        {/* ✅ WEBSITE LINK */}
                        {business.website && (
                            <a href={business.website.startsWith('http') ? business.website : `https://${business.website}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 mt-3 text-sm font-bold text-[#5856D6] hover:underline">
                                <i className="fa-solid fa-link"></i> {business.website.replace(/^https?:\/\//, '')}
                            </a>
                        )}
                    </div>
                </div>
            ...
            <div className="max-w-5xl mx-auto px-6 mt-10 grid md:grid-cols-3 gap-10">
                ...
                <div className="md:col-span-1">
                    <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 sticky top-24">
                        ...
                        {/* MAP (Embed if available, else Dynamic Map) */}
                        {business.googleMapEmbed ? (
                            <div className="rounded-xl overflow-hidden h-48 border border-slate-200 mb-4 bg-slate-100">
                                <div dangerouslySetInnerHTML={{ __html: business.googleMapEmbed }} className="w-full h-full [&_iframe]:w-full [&_iframe]:h-full [&_iframe]:border-0" />
                            </div>
                        ) : mapPins.length > 0 && (
                            <div className="rounded-xl overflow-hidden h-48 border border-slate-200 mb-4">
                                <GoogleDealMap pins={mapPins} />
                            </div>
                        )}

                        {/* LOCATION LIST (Use fallback if empty) */}
                        {(business.locations.length > 0 || (business.address && business.address.length > 0)) ? (
                            <ul className="space-y-3">
                                {business.locations.length > 0 ? (
                                    ...
                                    /* FALLBACK ADDRESS (If no extra locations added) */
                                    <li className="flex gap-3 text-sm text-slate-600">
                                        <i className="fa-solid fa-location-dot text-[#5856D6] mt-1"></i>
                                        <div>
                                            <div className="font-bold text-slate-900">Main Location</div>
            ...
            {/* 4. SWIPE REDEEM REMOVED per user request - only on Deal Page */}
